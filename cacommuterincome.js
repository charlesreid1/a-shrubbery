...
tl2 = L.tileLayer('http://api.tiles.mapbox.com/v4/mapbox.light/{z}/{x}/{y}.png?access_token=pk.eyJ1IjoiY2hhcmxlc3JlaWQxIiwiYSI6ImpreUJGM3MifQ.w5rSM7MjHv-SnOnt3gcqHA',{
    attribution: 'US Census Bureau',
    maxZoom: 18
});
tl2.addTo(m2);
...
function enhanceLayer2(f,l){
    var out = [];
    if (f.properties){
        l.bindPopup("County: "+f.properties['name']+"<br />"+"Percentage: "+(f.properties[key2]*100).toFixed());
        l.setStyle({    
...
function enhanceLayer3(f,l){
    var out = [];
    if (f.properties){
        l.bindPopup("County: "+f.properties['name']+"<br />"+"Percentage: "+(f.properties[key3]*100).toFixed());
        l.setStyle({    
